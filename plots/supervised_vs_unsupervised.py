...
ax.set_title('Überwachtes Lernen\n(mit Labels)', fontsize=12, fontweight='bold')
...
ax = axes[1]
ax.scatter(X_unsup[:, 0], X_unsup[:, 1], c='#3498DB', s=50, alpha=0.7,
           edgecolors='white', linewidth=0.5)
ax.set_title('Unüberwachtes Lernen\n(ohne Labels)', fontsize=12, fontweight='bold')
ax.set_xlabel('Feature 1: Betrag')
ax.set_ylabel('Feature 2: Frequenz')
ax.annotate('Cluster?', xy=(2, 4), fontsize=10, style='italic', color='#7F8C8D')
ax.annotate('Cluster?', xy=(-6, -8), fontsize=10, style='italic', color='#7F8C8D')
ax.annotate('Cluster?', xy=(-8, 2), fontsize=10, style='italic', color='#7F8C8D')
ax.grid(True, alpha=0.3)
